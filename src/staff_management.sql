...
SELECT staff_id, first_name, last_name, position AS role FROM staff;
...
WHERE 
    s.position = 'Trainer'  
    AND DATE(pts.session_date) BETWEEN DATE('now') AND DATE('now', '+30 days') -- Sessions within the next 30 days
GROUP BY 
